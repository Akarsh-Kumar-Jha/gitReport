        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2.5">
            {technologies.map((tech, idx) => (
              <div key={idx} className="bg-[#0A0A0A] text-[#F5F3EC] border-brutal px-3 py-2 font-mono text-xs flex items-center gap-2 shadow-brutal-sm">
                ...
                {typeof tech === 'object' && tech.purpose && (
                  <span className="text-[#999] text-[11px] border-l border-[#444] pl-2">
                    {tech.purpose}
                  </span>
                )}
              </div>
        ...
        <div className="space-y-3 pt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {important_components.map((comp, idx) => {
              ...
              return (
                <div key={idx} className="bg-[#F5F3EC] border-brutal p-3.5 space-y-1 shadow-brutal-sm">
                  <div className="flex items-center gap-2">
                    <span className="bg-[#0A0A0A] text-[#C8FF00] font-mono text-[10px] font-bold px-1.5 py-0.5 shrink-0">
                      {String(idx + 1).padStart(2, '0')}
                    </span>
                    <span className="font-mono font-bold text-xs text-[#0A0A0A] break-words">
                      {compName}
                    </span>
                  </div>
                  {compPurpose && (
                    <p className="text-xs text-[#5F5F5F] font-medium leading-relaxed pt-1">
                      {compPurpose}
                    </p>
                  )}
                </div>
        ...
        <div className="space-y-3 pt-2">
          ...
          <div className="space-y-2">
            {key_implementation_details.map((detail, idx) => (
              <div key={idx} className="bg-[#FFFFFF] border-brutal p-3.5 flex items-start gap-3 text-sm text-[#0A0A0A]">
                <span className="bg-[#0A0A0A] text-[#F5F3EC] font-mono text-[10px] font-bold px-1.5 py-0.5 shrink-0">
                  {idx + 1}
                </span>
